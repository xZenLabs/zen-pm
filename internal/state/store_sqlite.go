...
func (s *sqliteStore) migrate() error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS catalog_packages (
			...
			alpha_version TEXT NOT NULL DEFAULT ''
		)`,
	...
	alphaVersionExists, err := s.columnExists("catalog_packages", "alpha_version")
	if err != nil {
		return err
	}
	...
	if err := s.ensureColumn("catalog_packages", "alpha_version", "alpha_version TEXT NOT NULL DEFAULT ''"); err != nil {
		return err
	}
	if !versionsURLExists || !publishedAtExists || !releaseNotesExists || !prereleaseNotesExists || !alphaVersionExists {
...
func (s *sqliteStore) ReadCatalog() ([]CatalogEntry, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.Repo, &e.Priority, &e.Name, &e.Version, &platforms, &incompatiblePlatforms, &deps, &conflicts, &e.InstallURL, &e.UninstallURL, &e.Size, &e.Description, &e.Author, &tags, &e.IconURL, &e.RepoIconURL, &featured, &e.FeaturedImage, &featuredOrder, &e.Category, &e.Source, &e.SourceAsset, &e.SourceType, &e.SourceURL, &e.Stars, &e.Assets, &e.Constraints, &e.PluginModule, &pluginModuleAliases, &sourceAssetAliases, &e.ReadmeURL, &e.VersionsURL, &e.PublishedAt, &e.ReleaseNotesURL, &e.PrereleaseNotesURL, &e.PrereleaseVersion, &e.AlphaVersion); err != nil {
			return nil, err
		}
