...
function Search.draw(view, bb, x, y, w, value, placeholder, callback, clear_callback)
    ...
    if not P.image(bb, Images.asset("search.svg"), x + Theme.scale(14), y + math.floor((m.search_h - icon_size) / 2), icon_size, icon_size, { is_icon = true }) then
        P.text(bb, "Search", x + Theme.scale(14), y + Theme.scale(14), Theme.scale(46), "small", { bold = true })
    end
    ...
    if value and value ~= "" then
        P.text(bb, "x", x + w - Theme.scale(34), y + Theme.scale(12), Theme.scale(24), "heading", { bold = true })
        P.hit(view, x + w - Theme.scale(48), y, Theme.scale(48), m.search_h, clear_callback, "clear-search")
    end
    return m.search_h
end
